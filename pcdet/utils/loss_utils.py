...
class WeightedCrossEntropyLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        ...
        print('input:',input,input.shape,'target:',target,target.shape)
...
class PointSASALoss(nn.Module):
    ...
    def __init__(self,
                 func: str = 'BCE',
                 layer_weights: list = None, #[0.001, 0.01, 0.1, 0]
    ...
    def assign_target(self, points, gt_boxes): # gt_annos = kitti.get_label_annos(label_path, val_image_ids)
        """
        Args:
        ...
        for k in range(batch_size):
            ...
            point_cls_labels_single = point_cls_labels.new_zeros(bs_mask.sum()) # n(bs_mask.sum()) points each batch

            if not self.set_ignore_flag:
                box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(
                    points_single.unsqueeze(dim=0),
                    extend_gt_boxes[k:k + 1, :, 0:7].contiguous()).long().squeeze(dim=0)
                ...
                box_fg_flag = (box_idxs_of_pts >= 0)
#--------------------------------my-----------------------------------
                # box_fg_flag_easy = (box_idxs_of_pts >= 0) & (points_single[:,0] < 30)
                # box_fg_flag_mode = (box_idxs_of_pts >= 0) & (points_single[:,0] >= 30) & (points_single[:,0] < 50)
                # box_fg_flag_hard = (box_idxs_of_pts >= 0) & (points_single[:,0] >= 50)
# --------------------------------my-----------------------------------
...
# --------------------------------my-----------------------------------
#             point_cls_labels_single[box_fg_flag_easy] = 0.3
#             point_cls_labels_single[box_fg_flag_mode] = 0.6
#             point_cls_labels_single[box_fg_flag_hard] = 0.9
# --------------------------------my-----------------------------------
            point_cls_labels[bs_mask] = point_cls_labels_single
        return point_cls_labels # (N, ) 0: bg, 1: fg, -1: ignore (N=4096xB)
    ...
    def forward(self, l_points, l_scores, gt_boxes):
        ...
        """
        l_labels = []
        for i in range(len(self.layer_weights)): # len=4
    ...
    def loss_forward(self, l_scores, l_labels):
        ...
        for i in range(len(self.layer_weights)):
            ...
            # easy, mode, hard, negatives = li_labels == 1, li_labels == 2, li_labels == 3, li_labels == 0
            # cls_weights = easy * 1.0 + mode * 3.0 + hard * 10.0 + negatives * 1.0  # (N, 1)

            pos_normalizer = cls_weights.sum(dim=0).float()

            one_hot_targets = li_scores.new_zeros(*list(li_labels.shape), 2)
            one_hot_targets.scatter_(-1, (li_labels > 0).long().unsqueeze(-1), 1.0) #put 1 according to the labels (dim=-1)
#--------------------------------my---------------------------------------
            # one_hot_targets.scatter_(-1, (li_labels == 0.3).long().unsqueeze(-1), 0.3)
            # one_hot_targets.scatter_(-1, (li_labels == 0.6).long().unsqueeze(-1), 0.6)
            # one_hot_targets.scatter_(-1, (li_labels == 0.9).long().unsqueeze(-1), 0.9)
# --------------------------------my---------------------------------------
            one_hot_targets = one_hot_targets[:, 1:] # (N, 1)
            li_loss = self.loss_func(li_scores[None],
                                     one_hot_targets[None],
                                     cls_weights.reshape(1, -1))
            li_loss = self.layer_weights[i] * li_loss.sum() / torch.clamp(pos_normalizer, min=1.0)
