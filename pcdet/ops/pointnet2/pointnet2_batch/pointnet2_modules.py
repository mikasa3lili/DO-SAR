        ...
        if new_xyz is None:
            ...
            for i in range(len(self.sample_method_list)):
                ...
                if self.sample_method_list[i] == 'd-fps':
                    ...
                    scores_slice = scores[:, self.sample_range_list[i][0]:self.sample_range_list[i][1]].contiguous()
                ...
                elif self.sample_method_list[i] == 'do-fps':
                    assert scores is not None
                    scores_slice = scores[:, self.sample_range_list[i][0]:self.sample_range_list[i][1]].contiguous()
                    scores_slice = scores_slice.sigmoid() ** self.weight_gamma
                    #norm way:
                    # x_sig = torch.abs(xyz_slice[:, :, 0]) / 70
                    # scores_slice = x_sig * scores_slice
                    x_di = torch.abs(xyz_slice[:, :, 0]) / 70.4
                    x_mi = torch.floor(torch.div(xyz_slice[:, :, 0], 25))
                    a = torch.pow(x_di, x_mi) + x_mi
                    scores_slice = a * scores_slice
                    sample_idx = pointnet2_utils.furthest_point_sample_weights(
                        xyz_slice,
                        scores_slice,
                        self.npoint_list[i]
