...
class PointHeadVote(PointHeadTemplate):
    ...
    def assign_stack_targets_simple(self, points, gt_boxes, extend_gt_boxes=None, set_ignore_flag=True):
        ...
        targets_dict = {
            'point_cls_labels': point_cls_labels, #my: here, most fg points are belong to close objects.
    ...
    def assign_stack_targets_mask(self, points, gt_boxes, extend_gt_boxes=None,
        ...
        point_bs_labels = gt_boxes.new_zeros(points.shape[0])
        point_group_labels = gt_boxes.new_zeros(points.shape[0]).long()
        for k in range(batch_size):
            bs_mask = (bs_idx == k)
            points_single = points[bs_mask][:, 1:4]
            point_cls_labels_single = point_cls_labels.new_zeros(bs_mask.sum())
            ...
            point_group_labels_single = box_idxs_of_pts
            point_bs_labels[bs_mask] = k
            box_fg_flag = (box_idxs_of_pts >= 0)
            if set_ignore_flag:
            ...
            point_cls_labels[bs_mask] = point_cls_labels_single
            point_group_labels[bs_mask] = point_group_labels_single
        ...
        targets_dict = {
            ...
            'point_bs_labels': point_bs_labels,
    ...
    def assign_targets(self, input_dict):
        ...
        """
        assign_method = self.model_cfg.TARGET_CONFIG.ASSIGN_METHOD  # mask(yes) or iou
    ...
    def get_axis_aligned_iou_loss_lidar(self, pred_boxes: torch.Tensor, gt_boxes: torch.Tensor):#, cls:torch.Tensor):
        """
        Args:
    ...
    def get_cls_layer_loss(self, tb_dict=None):
        ...
        return point_loss_cls, cls_weights, point_cls_preds, tb_dict  # point_loss_cls: (N)

    def get_box_layer_loss(self, point_cls_preds, tb_dict=None):
        ...
        reg_weights = pos_mask.float()
        point_bs_labels = self.forward_ret_dict['point_bs_labels']
        point_group_labels = self.forward_ret_dict['point_group_labels']
        cls_mask = point_cls_scores[pos_mask].squeeze(-1)
        cls_save = cls_mask.new_zeros(cls_mask.shape[0])
        bs_mask = point_bs_labels[pos_mask].squeeze(-1)
        group_mask = point_group_labels[pos_mask].squeeze(-1)
        for i in range(0, int(point_bs_labels.shape[0]/256)):
            now_bs = (bs_mask==i)
            now_cls = cls_mask[now_bs]
            bs_group = group_mask[now_bs]
            a = cls_save[now_bs]
            each_group_num = Counter(bs_group.tolist())
            glist=[]
            for n in bs_group.tolist():
                if n not in glist:
                    glist.append(n)
            for k in glist:
                each_gid = (bs_group == k)
                cmax = torch.max(now_cls[each_gid])
                cmin = torch.min(now_cls[each_gid])
                cn = cls_mask[now_bs][each_gid]
                if each_group_num.get(k, 0) <= 4:
                    a[each_gid] = cn / cmax
                else:
                    a[each_gid] = cn # more stable
                    #a[each_gid] = (cn - cmin) / (cmax - cmin)
            cls_mask[now_bs] = a
        loss_weights_dict = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS
        if tb_dict is None:
            tb_dict = {}
        ...
        point_loss_offset_reg[pos_mask] *= (2*cls_mask)
        ...
        if isinstance(self.box_coder, box_coder_utils.PointBinResidualCoder):
            point_angle_cls_labels = point_reg_labels[:, 6:6 + self.box_coder.angle_bin_num]
    ...
    def get_loss(self, tb_dict=None):
        ...
        point_loss_cls, cls_weights, cls_pred, tb_dict_1 = self.get_cls_layer_loss()
        point_loss_box, box_weights, tb_dict_2 = self.get_box_layer_loss(cls_pred)
        ...
        point_loss = point_loss_box + point_loss_vote + point_loss_cls
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            ret_dict['point_bs_labels'] = targets_dict['point_bs_labels']
            ret_dict['point_group_labels'] = targets_dict['point_group_labels']
