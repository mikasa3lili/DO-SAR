...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True):
    ...
    if not os.path.exists(sequence_file): #sequence_file.exists():
        print('NotFoundError: %s' % sequence_file)
        return []

    dataset = tf.data.TFRecordDataset(str(sequence_file), compression_type='')
    dataset = dataset.apply(tf.data.experimental.ignore_errors())
    cur_save_dir = os.path.join(save_path, sequence_name) #save_path / sequence_name
    PosixPath(cur_save_dir).mkdir(parents=True, exist_ok=True)
    pkl_file = os.path.join(cur_save_dir, '%s.pkl' % sequence_name) #cur_save_dir / ('%s.pkl' % sequence_name)

    sequence_infos = []
    if os.path.exists(pkl_file): #pkl_file.exists():
    ...
    for cnt, data in enumerate(dataset):
        ...
        #num_points_of_each_lidar = save_lidar_points(frame, os.path.join(cur_save_dir, '%04d.npy' % cnt)) #cur_save_dir / ('%04d.npy' % cnt))
        num_points_of_each_lidar = save_lidar_points(frame, PosixPath(cur_save_dir) / ('%04d.npy' % cnt))
...
#
#     # print('Load record (sampled_interval=%d): %s' % (sampled_interval, sequence_name))
#     if not PosixPath(sequence_file).exists():
#         print('NotFoundError: %s' % sequence_file)
#         return []
#
#     dataset = tf.data.TFRecordDataset(str(sequence_file), compression_type='')
#     cur_save_dir = PosixPath(save_path) / sequence_name
#     cur_save_dir.mkdir(parents=True, exist_ok=True)
#     pkl_file = cur_save_dir / ('%s.pkl' % sequence_name)
#
#     sequence_infos = []
#     if PosixPath(pkl_file).exists():
#         sequence_infos = pickle.load(open(pkl_file, 'rb'))
